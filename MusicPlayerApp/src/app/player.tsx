...
export default function PlayerScreen() {
  const { vibe } = useLocalSearchParams();
  ...
  const buildVibeQueue = async () => {
    ...
    const allSongs = media.assets.map(t => ({
      id: t.id,
    ...
    try {
      ...
      const matched = allSongs.filter(s =>
        res.data.matched_songs.some(t => s.title.toLowerCase().includes(t.toLowerCase()))
      );
        ...
        renderItem={({ item, index }) => (
          <TouchableOpacity onPress={() => playSong(index)}>
            <Text style={[styles.queueItem, index === currentIndex && styles.activeItem]}>
              {index === currentIndex ? '▶️ ' : '    '}{item.title}
            </Text>
          </TouchableOpacity>
        )}
      />
