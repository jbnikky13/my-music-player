...
export default function VibeSelectorScreen({ navigation }) {
  ...
  const handleSelect = (vibe) => {
    setSelected(vibe);
    navigation.navigate('Player', { vibe });
  };
  return (
      ...
      {Object.entries(VIBES).map(([category, options]) => (
        <View key={category} style={styles.section}>
          ...
          <View style={styles.row}>
            {options.map((vibe) => (
              <TouchableOpacity key={vibe} style={[styles.btn, selected === vibe && styles.btnActive]} onPress={() => handleSelect(vibe)}>
                <Text style={[styles.btnText, selected === vibe && styles.btnTextActive]}>{vibe}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
